...
class Plain(DebiasLossFn):
    ...
    def forward(self, hidden, logits, bias, labels):
        ...
        #smoothing = 0.3
        # with torch.no_grad():
        # labels = labels * (1 - smoothing) + smoothing / 2274

        # print(logits)
        # loss = F.binary_cross_entropy_with_logits(logits, labels)
        # temperature = torch.nn.Parameter(torch.ones(0.9)).cuda()
        temperature = torch.nn.Parameter(
              torch.from_numpy(np.full((1,), 1, dtype=np.float32))).cuda()
        # print(temperature)
        loss = instance_bce(logits / temperature, labels)
